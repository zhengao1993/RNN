...
EEGData= rnn_utils_5.loadData(filePath)
EEGData_preprocessed = rnn_utils_5.preprocessData(EEGData)
(X_train, y_train), (X_test, y_test) = EEGData_preprocessed

(X_train_mini_batch, y_train_mini_batch) = rnn_utils_5.mini_batch(X_train, y_train, 0, 0)
(X_test_mini_batch, y_test_mini_batch) = rnn_utils_5.mini_batch(X_test, y_test, 0, 0)


model = init_model()
history = LossHistory()
model.fit(X_train_mini_batch, y_train_mini_batch, epochs=500, batch_size=512,
                callbacks=[history],
